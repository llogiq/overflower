    ...
    ($ty:ty, $name:ident) => {
        ...
        fn $name() {
            fn check(args: ($ty, $ty)) -> bool {
                ...
                let actual = catch_unwind(|| OverflowerAdd::add_panic(args.0, args.1)).ok();
                expected == actual
            }
            install_handler();
    ...
    ($ty:ty, $name:ident) => {
        ...
        fn $name() {
            fn check(args: ($ty, $ty)) -> bool {
                ...
                let actual = catch_unwind(|| OverflowerSub::sub_panic(args.0, args.1)).ok();
                expected == actual
            }
            install_handler();
            quickcheck(check as fn(($ty, $ty)) -> bool);
    ...
    ($ty:ty, $name:ident) => {
        ...
        fn $name() {
            fn check(args: ($ty, $ty)) -> bool {
                ...
                let actual = catch_unwind(|| OverflowerMul::mul_panic(args.0, args.1)).ok();
                expected == actual
            }
            install_handler();
            quickcheck(check as fn(($ty, $ty)) -> bool);
    ...
    ($ty:ty, $name:ident) => {
        ...
        fn $name() {
            fn check(args: ($ty, $ty)) -> bool {
                ...
                let actual = catch_unwind(|| OverflowerDiv::div_panic(args.0, args.1)).ok();
                expected == actual
            }
            install_handler();
            quickcheck(check as fn(($ty, $ty)) -> bool);
...
macro_rules! test_div_wrap {
    ($ty:ty, $name:ident) => {
        ...
        fn $name() {
            fn check(args: ($ty, $ty)) -> bool {
                let expected = args.0.checked_div(args.1);
                let actual = catch_unwind(|| OverflowerDiv::div_wrap(args.0, args.1)).ok();
                expected == actual
            }
            install_handler();
            quickcheck(check as fn(($ty, $ty)) -> bool);
    ...
    ($ty:ty, $name:ident) => {
        ...
        fn $name() {
            fn check(args: ($ty, $ty)) -> bool {
                ...
                let actual = catch_unwind(|| OverflowerRem::rem_panic(args.0, args.1)).ok();
                expected == actual
            }
            install_handler();
            quickcheck(check as fn(($ty, $ty)) -> bool);
...
macro_rules! test_rem_wrap {
    ($ty:ty, $name:ident) => {
        ...
        fn $name() {
            fn check(args: ($ty, $ty)) -> bool {
                let expected = args.0.checked_rem(args.1);
                let actual = catch_unwind(|| OverflowerRem::rem_wrap(args.0, args.1)).ok();
                expected == actual
            }
            install_handler();
            quickcheck(check as fn(($ty, $ty)) -> bool);
...
macro_rules! test_rem_saturate {
    ($ty:ty, $name:ident, $max:expr) => {
        ...
        fn $name() {
            fn check(args: ($ty, $ty)) -> bool {
                let expected = if args.1 == 0 { 0 } else { args.0 % args.1 };
    ...
    ($ty:ty, $name:ident, $bits:expr) => {
        ...
        fn $name() {
            fn check(args: ($ty, $ty)) -> bool {
                ...
                install_handler();
                let actual = catch_unwind(|| OverflowerShl::shl_panic(args.0, args.1)).ok();
                expected == actual
            }
            quickcheck(check as fn(($ty, $ty)) -> bool);
        }
